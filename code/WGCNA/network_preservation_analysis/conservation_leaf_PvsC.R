...
datColor = read.table("../TPM_log_leaf_C/gene_module_color.txt", sep="\t", check.names=FALSE)

modulecolor_ref = datColor[,3]

###########PvsC conservation analysis###########

multiExprA = list(A1 = list(data = t(datExprC)), A2 = list(data = t(datExprP)))
...
write.table(Obs.PreservationStats, file = "leaf_PvsC_Obs.PreservationStats.txt", sep="\t")
write.table(Z.PreservationStats, file = "leaf_PvsC_Z.PreservationStats.txt", sep="\t")


pdf("leaf_PvsC_conservation_z_score.pdf", height = 6, width = 10)
