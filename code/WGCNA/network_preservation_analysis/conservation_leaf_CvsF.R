...
datExprC = read.table("../TPM_log_leaf_C/TPM_log_leaf_C_filter.txt", sep="\t", head=TRUE, row.names=1, check.names=FALSE)
datExprP = read.table("../TPM_log_leaf_P/TPM_log_leaf_P_filter.txt", sep="\t", head=TRUE, row.names=1, check.names=FALSE)
datExprF = read.table("../TPM_log_leaf_F/TPM_log_leaf_F_filter.txt", sep="\t", head=TRUE, row.names=1, check.names=FALSE)

datColor = read.table("../TPM_log_leaf_F/gene_module_color.txt", sep="\t", check.names=FALSE)

modulecolor_ref = datColor[,3]

###########CvsF conservation analysis###########

multiExprA = list(A1 = list(data = t(datExprF)), A2 = list(data = t(datExprC)))
...
write.table(Obs.PreservationStats, file = "leaf_CvsF_Obs.PreservationStats.txt", sep="\t")
write.table(Z.PreservationStats, file = "leaf_CvsF_Z.PreservationStats.txt", sep="\t")


pdf("leaf_CvsF_conservation_z_score.pdf", height = 6, width = 10)
