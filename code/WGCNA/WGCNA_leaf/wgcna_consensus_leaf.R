...
#################
# This script provides the code to generate Consensus CoExpression analysis of leaf RNA-seq
...
outdir1 <- "leaf_consensus_result"
if (file.exists(outdir1) == FALSE){
  dir.create(outdir1)  
}

outdir2 <- "leaf_consensus_result/process"
...
# Col
Data1 <- read.table("/data2/usr/LiuWW/project_1/try/RNAseq_part3_r/WGCNA/TPM_log_leaf_C/TPM_log_leaf_C_filter.txt", sep="\t", stringsAsFactors=FALSE, head=TRUE, row.names=1, check.names = FALSE)

# Per
Data2 <- read.table("/data2/usr/LiuWW/project_1/try/RNAseq_part3_r/WGCNA/TPM_log_leaf_P/TPM_log_leaf_P_filter.txt", sep="\t", stringsAsFactors=FALSE, head=TRUE, row.names=1, check.names = FALSE)

# F1
Data3 <- read.table("/data2/usr/LiuWW/project_1/try/RNAseq_part3_r/WGCNA/TPM_log_leaf_F/TPM_log_leaf_F_filter.txt", sep="\t", stringsAsFactors=FALSE, head=TRUE, row.names=1, check.names = FALSE)
...
# Col
C_output <- read.table("/data2/usr/LiuWW/project_1/try/RNAseq_part3_r/WGCNA/TPM_log_leaf_C/gene_module_color.txt", sep="\t")
row.names(C_output) <- C_output[,1]
C_mods <- C_output[row.names(data),3]

# Per
P_output <- read.table("/data2/usr/LiuWW/project_1/try/RNAseq_part3_r/WGCNA/TPM_log_leaf_P/gene_module_color.txt", sep="\t")
row.names(P_output) <- P_output[,1]
P_mods <- P_output[row.names(data),3]

# F1
F_output <- read.table("/data2/usr/LiuWW/project_1/try/RNAseq_part3_r/WGCNA/TPM_log_leaf_F/gene_module_color.txt", sep="\t")
...
multiExpr = list(Col = list(data=t(data[,1:45])), Per = list(data=t(data[,46:90])), F1 = list(data=t(data[,91:135])))
...
write.table(temp, file = paste(outdir1, "leaf_CM_GO_merge.txt", sep = "/"),
            sep = "\t", row.names = FALSE, col.names = TRUE)
# plot heat map
temp[,-1][!is.na(temp[,-1])] <- -log10(temp[,-1][!is.na(temp[,-1])])
##sort
library(tidyverse)
temp <- dplyr::arrange(temp, desc(yellow), desc(green), desc(turquoise), desc(red), desc(blue), desc(brown), desc(grey))
##plot
suppressPackageStartupMessages(library(ComplexHeatmap))
suppressPackageStartupMessages(require(circlize)) 
suppressPackageStartupMessages(library(dendextend))
...
pdf(file = paste(outdir1, "Fig2G_leaf_CM_GO.pdf", sep = "/"), width = 6, height = 16)
...
write.table(ego_result, file = paste(outdir1, "leaf_CM_hub_union_GO.txt", sep = "/"), 
            sep = "\t", row.names = FALSE, col.names = TRUE)
emapplot(ego, 
         showCategory = 200,
         color = "p.adjust",
...
# TOMplot
#################
setwd("/data2/usr/LiuWW/project_1/try/RNAseq_part3_r/WGCNA2/leaf_consensus_result")
for (set in 1 : nSets) {
